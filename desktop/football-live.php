<?php
	...
	// CHECK THE GLIVE MATCHID

	if($infoRow->match_code == "")
	{
		$infoGlive = tep_fetch_object(tep_query("SELECT * FROM nano_live_match_list WHERE MatchID = '&".tep_input($_GET['id'])."&'"));

		$infoRow->match_homeEn = $infoGlive->Home;
		$infoRow->match_awayEn = $infoGlive->Away;

		$infoRow->league_nameEn = $infoGlive->League;
		$infoRow->match_leagueEn = $infoGlive->League;
		$infoRow->match_time = $infoGlive->TimeStart;

		$infoRow->NowPlaying = $infoGlive->NowPlaying;
		$infoRow->Channel = $infoGlive->Channel;

		if($infoGlive->NowPlaying == 1){
			$todayDate = date_create(date("Y-m-d H:i:s"));
			$startDate = date_create($infoGlive->TimeStart);
			...
			$infoRow->match_state = $mins + $remainingMins . "'";

		}else if($infoGlive->NowPlaying == 0 && $infoGlive->TimeStart > date("Y-m-d H:i:s")){
			$infoRow->match_state = "Not started";
		}else if($infoGlive->NowPlaying == 0 && $infoGlive->TimeStop < date("Y-m-d H:i:s")){
			$infoRow->match_state = "FT";
		}
	}
						...
						<div class="live-streaming">
							<center>
								...
								<div class="video-wrap">
									<div style="position: relative; background: url('includes/images/football-background.png') no-repeat center center;width: 100%;height: 100%;">
